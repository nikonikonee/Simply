...
6. **themida_capture.** At OEP, freeze every target thread, then plant `0xCC` on the first byte of every export in every loaded external module. For each themida API stub, spawn a suspended remote thread at the stub VA, arm `DR0` at that same VA so the single-step anchors `rsp`, resume, and let the CC carpet pin the landing API in one `EXCEPTION_BREAKPOINT` event. Scaffolding calls (stub calls a helper that calls the api) get classified by comparing current `rsp` to `anchor_rsp` and checking where `[rsp]` points: restore the byte, `TF` one step, replant. `.text` is stripped to `PAGE_READONLY` for the duration so a stub that dispatches into user code (one of the poisoned slots literally runs `main`) faults cleanly and we abandon it. Writable pages still get snapshotted and restored since CCs firing during CRT init dirty `.data`. Produces a `stub -> api` map plus a `slot -> stub` map for poisoned IAT entries. ~60s with TF-stepping, ~700ms with the CC carpet.
...
9. **themida_stubs.** Find every `call`/`jmp rel32` in `.text` that lands in a packer section. Peel the `FF25`/`E9`/`EB` chain statically, or fall back to the runtime `stub -> api` map for VM-Macro stubs. Resolved sites get rewritten to jump through a 6-byte `FF25` trampoline emitted in `.simply` slack. Unresolvable sites (EPV thunks themida planted in `.text` to redirect the original EP, dead once `AddressOfEntryPoint` is flipped to the real OEP) get their 5-byte instruction overwritten with `0xCC`, which drops the last xref so `section_cleaner` can remove `.themida`.
